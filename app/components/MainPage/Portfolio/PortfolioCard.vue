<template>
  <div class="portfolio-card group relative bg-white rounded-2xl overflow-hidden shadow-lg hover:shadow-2xl transition-all duration-500 cursor-pointer transform hover:-translate-y-2"
    ...
    <div class="relative h-64 overflow-hidden">
      ...
      <!-- Category badge -->
      <div class="absolute top-4 left-4 px-3 py-1 rounded-full text-xs font-medium transition-all duration-300 group-hover:scale-110"
           :class="categoryStyles">
        {{ project.category }}
      </div>
      
      <!-- Status badge -->
      <div v-if="project.isLive" 
        ...
        <Icon name="ic:round-visibility" class="w-3 h-3 inline mr-1" />
        Live
      </div>
      ...
      <div class="absolute bottom-4 right-4 flex gap-2">
        <button v-if="project.liveUrl" 
          ...
          <Icon name="ic:round-launch" class="w-4 h-4 text-gray-700" />
        </button>
        <button @click.stop="$emit('view-details', project)"
          ...
          <Icon name="ic:round-info" class="w-4 h-4 text-white" />
        </button>
      </div>
    ...
    <div class="p-6">
      ...
      <h3 class="text-xl font-bold text-gray-900 mb-2 transition-all duration-300 group-hover:text-blue-600 group-hover:transform group-hover:scale-105 group-hover:translate-x-1">
        {{ project.title }}
      </h3>
      ...
      <div class="flex flex-wrap gap-1 mb-4">
        <span v-for="(tech, index) in project.technologies.slice(0, 4)" 
              :key="tech"
      ...
      <div class="flex items-center justify-between text-xs text-gray-500 mb-4 transition-all duration-300 group-hover:text-gray-700">
        <div class="flex items-center gap-4">
          <div class="flex items-center gap-1 transition-transform duration-300 group-hover:scale-105">
            <Icon name="ic:round-schedule" class="w-3 h-3 transition-transform duration-300 group-hover:rotate-12" />
            {{ project.duration }}
          </div>
          <div class="flex items-center gap-1 transition-transform duration-300 group-hover:scale-105">
            <Icon name="ic:round-people" class="w-3 h-3 transition-transform duration-300 group-hover:rotate-12" />
        ...
        <div class="flex items-center gap-1 transition-all duration-300 group-hover:scale-110 group-hover:text-yellow-600">
          <Icon name="ic:round-star" class="w-3 h-3 text-yellow-500 transition-transform duration-300 group-hover:rotate-12" />
      ...
      <div class="flex items-center justify-between border-t border-gray-100 pt-4 group-hover:border-gray-200 transition-all duration-300">
        <div class="flex items-center gap-3">
          <div class="w-8 h-8 bg-gradient-to-r from-blue-500 to-purple-500 rounded-full flex items-center justify-center transition-all duration-300 group-hover:scale-110 group-hover:rotate-6 group-hover:shadow-lg">
        ...
        <!-- Arrow indicator -->
        <div class="w-8 h-8 bg-gray-100 group-hover:bg-blue-100 rounded-full flex items-center justify-center transition-all duration-300 group-hover:scale-110 group-hover:rotate-45">
          <Icon name="ic:round-arrow-forward" class="w-4 h-4 text-gray-600 group-hover:text-blue-600 transition-all duration-300 group-hover:-rotate-45" />
        </div>
      </div>
    </div>
...
const glowColor = computed(() => {
  const colors = {
    ...
    'Fintech': 'bg-yellow-400'
  };
  return colors[props.project.category as keyof typeof colors] || 'bg-gray-400';
});
